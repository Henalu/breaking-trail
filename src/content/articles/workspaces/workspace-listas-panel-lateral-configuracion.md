...
sys_ux_list_menu_config        ← Contenedor principal (1 por Workspace)
  └── sys_ux_list_category     ← Secciones del menú (Tasks, Requests, My Work...)
        └── sys_ux_list        ← Listas concretas con tabla, query y ordenación
              └── sys_ux_applicability_m2m  ← Audiencias que pueden ver cada lista
```
...
**`sys_ux_applicability_m2m` — List Applicability**
...
En `sys_ux_applicability_m2m`, crea una entrada por lista indicando qué audiencia puede verla:
...
3. Si no aparece: revisa `sys_ux_applicability_m2m` primero (es la causa más frecuente)
...
Revisa `sys_ux_applicability_m2m`. La audience asignada puede no incluir a todos los usuarios esperados. Verifica los roles que componen esa audience.
